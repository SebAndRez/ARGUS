  ...
  >(
    () => ({
      ...
      osmCriticalInfrastructure: {
        count: 0,
        detail: "OSM/Overpass contextual; no incidentes ni bulk global",
        status: "idle",
      },
      hdxHapiHumanitarianContext: {
        count: 0,
        detail: "Contexto humanitario HAPI; requiere HAPI_APP_IDENTIFIER",
        status: "idle",
      },
      whoDiseaseOutbreakNews: {
        count: 0,
        detail: "Brotes oficiales WHO DON; ubicacion aproximada",
        status: "idle",
      },
      ecdcPublicHealthThreats: {
        count: 0,
        detail: "Amenazas sanitarias EU/EEA; CDTR como evidencia",
        status: "idle",
      },
      gdeltMediaSignals: {
        count: 0,
        detail: "Senales OSINT/media; no confirma incidentes",
        status: "idle",
      },
      copernicusGlofasFloodForecast: {
        count: 0,
        detail: "Forecast/modelo de inundacion; requiere EWDS",
        status: "idle",
      },
      copernicusGfmObservedFloodExtent: {
        count: 0,
        detail: "Flood extent satelital; requiere token GFM",
        status: "idle",
      },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/92 px-4 py-3 backdrop-blur-xl xl:px-8">
        ...
        />
        <nav className="mt-3 flex flex-wrap gap-2" aria-label="Accesos ARGUS">
          <a
            href="/app/como-usar"
            className="rounded border border-cyan-300/20 bg-cyan-400/10 px-3 py-2 text-xs font-semibold text-cyan-100 hover:bg-cyan-400/20"
          >
            Como usar ARGUS
          </a>
          <a

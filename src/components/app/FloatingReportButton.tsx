...
export default function FloatingReportButton({ onClick, disabled }: FloatingReportButtonProps) {
  return (
    <button
      ...
      disabled={disabled}
      className={`fixed bottom-12 right-4 z-50 flex h-14 min-w-36 items-center gap-3 rounded-full border px-3 pr-5 text-white backdrop-blur-xl transition duration-200 focus:outline-none focus:ring-2 focus:ring-cyan-300/80 focus:ring-offset-2 focus:ring-offset-slate-950 ${
        disabled
